#!/usr/bin/env python3
"""
N8N Monitoring Script for 52Pi Fan HAT (Clean Design)
- OLED Display: N8N status + system stats (borderless)
- LED Indicators: System, N8N health, network, temperature
...
class N8NMonitor:
    ...
    def check_n8n_health(self):
        ...
        try:
            ...
            result = subprocess.run(['docker', 'ps', '--filter', 'name=n8n', '--format', '{{.Status}}'], 
                                  capture_output=True, text=True)
            container_running = 'Up' in result.stdout
            
            # Test n8n connection (401 is OK - means auth is working)
            response = requests.get('http://localhost:5678', timeout=5)
            n8n_responding = response.status_code in [200, 401]
            
            return container_running and n8n_responding
        except:
            return False
    ...
    def update_oled(self, stats, n8n_healthy, network_ok):
        ...
        with canvas(self.oled) as draw:
            # Header
            status = "RUNNING" if n8n_healthy else "ERROR"
            draw.text((0, 0), f"N8N {status}", fill="white")
    ...
    def control_leds(self, stats, n8n_healthy, network_ok):
    ...
    def run_monitoring_loop(self):
        ...
        while True:
            try:
                ...
                self.update_oled(stats, n8n_healthy, network_ok)
                self.control_leds(stats, n8n_healthy, network_ok)
                
                # Log every 30 seconds
                if int(time.time()) % 30 == 0:
                    print(f"N8N: {'OK' if n8n_healthy else 'ERROR'} | "
